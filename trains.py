...
parser.add_argument(
    ...
    type=str,
    default='ResNet18',
    choices=['ResNet18', 'VGG16_BN', 'WRN28_10', 'PyramidNet110'],
...
parser.add_argument('--schedule',  metavar='SCHEDULE', default='cosine', type=str,
...
parser.add_argument('--adaptive', action='store_true',
                    help='Enable Adaptive mode (ASAM / FE-ASAM)')
...
parser.add_argument('--T', default=0.1, type=float,
                    metavar='T', help='temperature for FE-SAM')
parser.add_argument('--beta', default=0.9, type=float,
                    metavar='B', help='EMA decay rate for HSAM gradient memory (default: 0.9)')
...
def main():
    ...
    if args.optimizer == 'SAM':
        ...
        optimizer = SAM(model.parameters(), base_optimizer, rho=args.rho, adaptive=args.adaptive, lr=args.lr, weight_decay=args.weight_decay)        
    elif args.optimizer == 'FESAM':
        base_optimizer = torch.optim.SGD
        optimizer = FESAM(model.parameters(), base_optimizer,
                      rho=args.rho, T=args.T, adaptive=args.adaptive,
                      lr=args.lr, momentum=args.momentum,
                      weight_decay=args.weight_decay, nesterov=False)

    elif args.optimizer == 'FESAM_adamw':
        base_optimizer = torch.optim.AdamW
        optimizer = FESAM(model.parameters(), base_optimizer,
                      rho=args.rho, T=args.T, adaptive=args.adaptive,
    ...
    print('ori train acc: ', ori_train_acc)
...
def train(train_loader, model, criterion, optimizer, lr_scheduler, epoch):
    global train_loss, train_err, post_train_loss, post_train_err
    global ori_train_loss, ori_train_err, arr_time, train_acc, ori_train_acc
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        # --- FE-SAM branch ---
        if args.optimizer.startswith('FESAM'):
            def closure():
                optimizer.zero_grad()
            ...
            predictions = model(input_var).detach()
            output_adv = predictions
            loss_adv = loss
        ...
        total_err += (output.max(1)[1] != target_var).sum().item()

        ori_total_loss += loss_adv.item() * input_var.size(0)
        ori_total_err += (output_adv.max(1)[1] != target_var).sum().item()
    ...
    epoch_err = total_err / len(train_loader.dataset)
    epoch_acc = 1.0 - epoch_err

    ori_epoch_loss = ori_total_loss / len(train_loader.dataset)
    ori_epoch_err = ori_total_err / len(train_loader.dataset)
    ori_epoch_acc = 1.0 - ori_epoch_err

    print(f"Total time : {batch_time.sum:.3f}")
    print(f"Train Loss: {epoch_loss:.4f}, Train Accuracy: {epoch_acc:.4f}")

    # save stats
    train_loss.append(round(epoch_loss, 4))
    train_err.append(round(epoch_err, 4))
    train_acc.append(round(epoch_acc, 4))

    ori_train_loss.append(round(ori_epoch_loss, 4))
    ori_train_err.append(round(ori_epoch_err, 4))
    ori_train_acc.append(round(ori_epoch_acc, 4))

    arr_time.append(round(batch_time.sum, 2))

    if args.wandb:
        wandb.log({
            "train loss": epoch_loss,
            "train acc": epoch_acc,
            "perturbed train loss": ori_epoch_loss,
            "perturbed train acc": ori_epoch_acc
        })
...
def validate(val_loader, model, criterion):
    ...
    epoch_err = total_err / len(val_loader.dataset)
    epoch_acc = 1.0 - epoch_err

    print(f'Test Loss : {epoch_loss:.4f}, Test Accuracy : {epoch_acc:.4f} \n')
    

    test_loss.append(round(total_loss / len(val_loader.dataset), 4))
    test_err.append(round(total_err / len(val_loader.dataset), 4))
    test_acc.append(round(1 - total_err / len(val_loader.dataset), 4))
