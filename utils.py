...
import os
import time
from vgg import VGG, VGG16, VGG16BN, VGG19, VGG19BN
from resnet import ResNet
from wide_resnet import WideResNet, WideResNet28x10, WideResNet16x8
...
def get_model(args):
    ...
    if args.arch == 'VGG16_BN':
        return VGG(num_classes=num_classes, **VGG16BN.kwargs)
        ...
        return WideResNet(num_classes=num_classes, **WideResNet28x10.kwargs)
    elif args.arch == 'PyramidNet110':
        return PyramidNet(110, 270, num_classes)

    # ===== ImageNet pretrained =====
    if args.datasets == 'ImageNet':
        return models_imagenet.__dict__[args.arch](num_classes=num_classes)
...
class FESAM(torch.optim.Optimizer):
    def __init__(self, params, base_optimizer, rho=0.05, T=0.1, adaptive=False, **kwargs):
        assert rho >= 0.0, f"Invalid rho, should be non-negative: {rho}"
        assert T >= 0.0, f"Invalid temperature T, should be non-negative: {T}"

        defaults = dict(rho=rho, T=T, adaptive=adaptive, **kwargs)
        super(FESAM, self).__init__(params, defaults)
    ...
    def first_step(self, grad_norm):
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                # Adaptive or non-adaptive perturbation
                e_w = (torch.pow(p, 2) if group["adaptive"] else 1.0) * p.grad * scale.to(p)
                p.add_(e_w)

    @torch.no_grad()
    def second_step(self, L0, L_eps):
        T = self.param_groups[0]["T"]

        if T == 0:
            alpha = 1.0
        else:
            delta = L_eps.item() - L0.item()
            alpha = 1.0 / (1.0 + torch.exp(-torch.tensor(delta / T, device=L0.device)))
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                g0 = self.state[p]["old_g"]
                g_eps = p.grad.clone()
                p.data = old_p
                # blend original and perturbed gradient
                p.grad = alpha * g_eps + (1 - alpha) * g0
    ...
    @torch.no_grad()
    def step(self, closure=None):
        assert closure is not None, "FE-SAM requires closure to compute loss"
        with torch.enable_grad():
            L0 = closure()
        grad_norm = self._grad_norm()
        self.first_step(grad_norm)
        with torch.enable_grad():
            L_eps = closure()
        self.second_step(L0, L_eps)
        return L0
